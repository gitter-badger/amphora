...
/**
 * Converts a stream into a promise by merging all the data events together.
 * @param {Stream} pipe
 * @returns {Promise}
 */
function pipeToPromise(pipe) {
  var str = [],
    d = bluebird.defer();
  pipe.on('data', function (data) { str.push(data); })
    .on('error', d.reject)
    .on('end', function () {
      d.resolve(str.join(''));
    });
...
/**
 * Typically used for the @list special endpoint
 *
 * @returns {function}
 */
function tieredList() {
  return function (req, res) {
    var prevPromise, currentPromise,
      uri = references.replaceVersion(req.uri),
      query = req.query,
      from = query.from,
      size = query.size && parseInt(query.size),
      listOptions = {
        gte: uri + '\x00',
        lte: uri + '\xff',
        values: false
    ...
    // from, size, sort, list, next, prev; all four letter words

    if (size && !isNaN(size)) {
      listOptions.limit = size + 1;
      console.log('size', size, 'from', from);
      if (from) {
        listOptions.gte = from; // inclusive

        prevPromise = pipeToPromise(db.list({
          limit: size,
          reverse: true,
          gte: uri + '\x00',
          lt: from, // not inclusive
          values: false
        })).then(JSON.parse);
      }
    }

    currentPromise = pipeToPromise(db.list(listOptions)).then(JSON.parse);

    bluebird.all([
      currentPromise,
      prevPromise
    ]).spread(function (list, prev) {
      var result = {};

      if (size && !isNaN(size) && list.length > size) {
        result.next = uri + '@list?size=' + size + '&from=' + _.last(list);
        list.length = size;
      }
      ...
      if (prev && prev.length > 0) {
        result.prev = uri + '@list?size=' + size + '&from=' + _.last(prev);
      }
...
module.exports.tieredList = tieredList;

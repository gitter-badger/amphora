 ...
 * @returns {Promise}
 */
function getPutOperations(uri, data, prefix) {
  ...
  return bluebird.map(splitCascadingData(uri, data), function (op) {
    ...
    return put(op.key, op.value, prefix); // This is where if calls the component's server.js.
  }).then(function (ops) {
 ...
 * @returns {Promise}
 */
function cascadingPut(uri, data, prefix) {

  // split data into pieces
  return getPutOperations(uri, data, prefix)
    .then(function (ops) {
      console.log('These are the locals for the put', prefix);
